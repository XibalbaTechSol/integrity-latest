...
export const RegistryExplorer: React.FC<RegistryExplorerProps> = ({ isOpen, onClose, initialQuery }) => {
    ...
    // DID/address the oracle can resolve on its own (XNS resolution is an
    // on-chain XibalbaNameService read, not an oracle route), so we pass
    // the query through as-is and let the oracle's own 404 explain that.
    const handleResolve = async () => {
        ...
        try {
            const id = query.trim();
            const agentRes = await fetch(`${ORACLE_URL}/v1/agent/${encodeURIComponent(id)}`);
            if (agentRes.status === 404) {
                setError("Agent not found in the Integrity Registry. This address/DID has not been registered.");
                return;
            }
            if (!agentRes.ok) {
            ...
            setResult({
                ...
                has_eth_address: agent.has_eth_address,
                                    ...
                                    <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? '20px' : '0', marginBottom: '24px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                            ...
                                            <div style={{ overflow: 'hidden' }}>
                                                ...
                                                <p style={{
                                                    ...
                                                    {result.has_eth_address ? 'EVM address on file' : 'No EVM address on file'} · resolved from {result.primitives_source}
                                                </p>
                                            </div>
                                        </div>
                                        <div style={{ textAlign: isMobile ? 'left' : 'right' }}>
                                            <div style={{
                                                ...
                                                color: result.ais === null ? 'rgba(255,255,255,0.3)' : result.ais >= 750 ? 'var(--gold)' : result.ais >= 500 ? '#60a5fa' : '#f43f5e'
                                            }}>
                                                {result.ais === null ? '—' : Math.round(result.ais)}
                                            </div>
