...
import { AuditLogsPanel } from '../components/diagnostics/AuditLogsPanel';
...
export const SystemDiagnosticsPage = () => {
  ...
  return (
    <div className="main-content" style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      <TopBar title="System Diagnostics">
        {selectedAgent && <span className="badge badge-primary">{selectedAgent.id}</span>}
      </TopBar>

      <div className="page-content custom-scrollbar" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', padding: '0 24px 24px' }}>
        {/* Tri-Metric Display */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--space-6)', flexShrink: 0 }}>
          {[
        ...
        <div className="card" style={{ flexShrink: 0 }}>
          ...
          {error && <div style={{ color: 'var(--danger)', fontSize: '0.85rem', marginTop: '8px' }}>{error}</div>}
        ...
        {/* One unified, filterable, real event log -- see AuditLogsPanel's
            own doc comment: merges BCC decisions + SDK telemetry + OTel
            spans into a single table for real manual debugging, not split
            across tabs. A firm min-height guarantees the table is always
            tall enough to show many rows even when the metrics + volume
            chart above it are on screen -- otherwise the log gets squeezed
            to a header-row-only sliver below the fold (the whole page now
            scrolls, so this height is additive, not clipped). */}
        <div style={{ flexShrink: 0, minHeight: '620px', display: 'flex', flexDirection: 'column' }}>
          <AuditLogsPanel />
        </div>
      </div>
    </div>
