// forked from design-sync lib/bundle.mjs — added a '.ttf': 'dataurl' esbuild
// loader mapping. integrity-mvp's synthesized entry re-exports every src/
// file, which pulls in TriMetricWidget.tsx's `import 'katex/dist/katex.min.css'`;
// that stylesheet references raw .ttf fallback fonts, and esbuild has no
// default loader for .ttf (only .svg/.png/.woff/.woff2 were mapped upstream),
// so the whole bundle failed with 20 "No loader is configured" errors. This
// is purely an asset-loader addition — it doesn't change the emitted output
// contract (manifest shape, @dsCard markers, etc.) that the "don't fork
// bundle.mjs/emit.mjs" guidance protects.
