// forked from design-sync lib/common.mjs — added "excludeSrcFiles" to
// CONFIG_KEYS so the strict validator accepts it. Paired with the
// lib/source-kit.mjs fork (see cfg.libOverrides / NOTES.md for why).
// This fork is loaded only for package-build.mjs's own top-level
// `loadLib('common')` pre-flight check — every other lib module still
// resolves its own './common.mjs' import to the unforked original, so this
// doesn't fork validateConfig's behavior anywhere else in the pipeline.
//
...
export const CONFIG_KEYS = new Set([
  ...
  'storybookConfigDir', 'storybookStatic', 'srcDir', 'tsconfig', 'excludeSrcFiles', 'includeSrcDirs',
  // styling + assets
