// forked from design-sync lib/source-kit.mjs — added cfg.excludeSrcFiles.
// integrity-mvp has no library dist/, so every build runs the synth-entry
// fallback below, which does `export * from <file>` for EVERY .tsx/.jsx
// under srcRoot with no filter besides the stories/test/spec regex.
// cfg.componentSrcMap: null only removes a NAME from the derived component
// list — it does not stop the named file from being pulled into the
// synthesized entry (and therefore the bundle), because that filtering
// happens after entry construction. That mattered concretely here:
// src/components/ui/MermaidDiagram.tsx statically imports the whole
// `mermaid` package (mermaid-parser + cytoscape + ~40 per-diagram-type
// chunks + its own bundled katex), which alone pushed _ds_bundle.js past
// the platform's 12MB upload cap. componentSrcMap:null on "MermaidDiagram"
// left the file (and ~5-6MB of transitively bundled deps) in the output.
// cfg.excludeSrcFiles (repo-relative paths) filters srcFiles BEFORE entry
// synthesis, so an excluded file's imports never reach the bundler at all.
//
...
export async function resolvePackage(ctx) {
  ...
  const excludeSrc = new Set((cfg.excludeSrcFiles ?? []).map((p) => slash(resolve(PKG_DIR, p))));
  const includeDirs = (cfg.includeSrcDirs ?? []).map((p) => slash(resolve(PKG_DIR, p)) + '/');
  const srcFilesAll = srcRoot ? walk(srcRoot, (n) => /\.(tsx|jsx|mdx?)$/.test(n)) : [];
  let srcFiles = srcFilesAll;
  if (includeDirs.length) {
    const before = srcFiles.length;
    srcFiles = srcFiles.filter((p) => includeDirs.some((d) => p.startsWith(d)));
    console.error(`  [INCLUDE_SRC] ${srcFiles.length}/${before} srcFiles kept — under one of ${includeDirs.length} cfg.includeSrcDirs`);
  }
  {
    const before = srcFiles.length;
    srcFiles = srcFiles.filter((p) => !excludeSrc.has(p));
    if (excludeSrc.size) {
      const matched = before - srcFiles.length;
      console.error(`  [EXCLUDE_SRC] ${matched}/${excludeSrc.size} cfg.excludeSrcFiles matched and dropped from srcFiles`);
      if (matched < excludeSrc.size) {
        const found = new Set(srcFilesAll.filter((p) => excludeSrc.has(p)));
        for (const p of excludeSrc) if (!found.has(p)) console.error(`  [EXCLUDE_SRC] no match for "${p}" — check the path`);
      }
    }
  }
